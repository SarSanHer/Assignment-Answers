...
report_file = File.open('./New_report.txt', "a+")
report_file.puts "\n\nSARA SANCHEZ-HEREDERO MARTÍNEZ \n\nASSIGNMENT 2 OUTPUT\n\nNetworks depth: 2"
report_file.puts "\n\nInteractions filters: Species (only Arabidopsis thaliana) and Intact MI-score (above 0.485)"
#report_file.puts "\n\nThere is a total of #{for_report.keys.to_a.lenght} networks \n\n\t\t\t\t      *** *** ***\n\n\n\n"

for_report.keys.each do |key|
  i+=1
  report_file.puts "\nNetwork #{i}, with ID '#{key}', and has the following genes:\n\n"
  for_report[key].each do |gene|
    ...
    report_file.puts "\t\t 6) Interactions: #{($annotated_members_array[0].get_info(gene)[5])&genes_list}"
    report_file.puts "\n\n\n\n"
      end
  report_file.puts "\n\n\n\n\t\t\t\t      *** *** ***\n\n\n\n"
end                                                                                                                                
